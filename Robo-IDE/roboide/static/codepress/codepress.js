...
CodePress = {
	...
	edit : function() {
		...
		if(!arguments[1]||arguments[1]=='') { // file name of the source code (to open from server)
			this.setLanguage();
            //Changed to use /filesrc by SE 2007-02-06
			//cpEditor.src = cpPath+'modules/codepress.php?action=edit&file='+this.fileName+'&language='+this.language+'&engine='+cpEngine
            cpEditor.src = '/filesrc?action=edit&file='+this.fileName+'&language='+this.language+'&engine='+cpEngine;
			return;
		}
	...
	setContent : function() {
        ...
        //This line also changed! '+cpPath+'/modules/codepress.php >
        ///filesrc
        //This made the initial load work
		$('codepress').innerHTML = '<div id="cp-window">'+
			'<iframe id="cp-editor" src="/filesrc?engine='+cpEngine+'&file='+ (onLoadEdit ? 'null' : this.fileName) +'&language='+this.language+'" style="height:'+ cpEditorHeight +'px"></iframe>'+
			'<div id="cp-menu">'+
