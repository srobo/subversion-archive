...
void mputcharUSART(unsigned char data){
   //TODO: Fill with buffer checking code
...
unsigned char mgetsUSART(unsigned char *buffer, unsigned char len){
	...
	if(usart_in_head > usart_in_tail)
		...
		lenavailable = USART_IN_BUFFER_SIZE - (usart_in_head - usart_in_tail);
		
	len = (len < lenavailable) ? len : lenavailable;
	
	for(pos = 0; pos < lenavailable; pos++){
