...
class SocketController:
    ...
    def process(self, data):
        ...
        for team, output in data:
            if not team in sims:
                logging.error("Received data for unknown team")
                continue
            
            if not isinstance(output, list):
                logging.error("Team data not in the form of a list")
                continue

            if sims[team]["state"] == "STARTED":
                logging.debug("Got first info for team %d" % team)
                sims[team]["state"] = "RECEIVING"

            if len(output) == 0:
                logging.error("Zero length output for team %d" % team)
                continue
